...
function spawn_enemy() {  
  ...
  if (spawn_zone != null) {
    enemy_timer = 0;
    enemies.push(new Enemy(createVector(randomIntFromInterval(spawn_zone[0].x, spawn_zone[1].x), randomIntFromInterval(spawn_zone[0].y, spawn_zone[1].y))));
  }
}
...
class Enemy {
  constructor(position){
    ...
    this.velocity = createVector(0.0, 0.0);
    this.size = 5;
    this.speed = randomIntFromInterval(1, player.speed_limit * 0.5);
